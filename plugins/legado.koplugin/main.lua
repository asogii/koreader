...
function Legado:doRegister(username, password, menu)
...
function Legado:doLogin(username, password, menu)
    local KOSyncClient = require("KOSyncClient")
    local client = KOSyncClient:new{
        custom_url = self.settings.custom_server,
        service_spec = self.path .. "/api.json"
...
function Legado:getLastPercent()
...
function Legado:getLastProgress()
    if self.ui.document.info.has_pages then
...
function Legado:getDocumentDigest()
...
function Legado:getFileDigest()
    return self.ui.doc_settings:readSetting("partial_md5_checksum")
end

function Legado:getFileNameDigest()
...
function Legado:syncToProgress(progress)
...
function Legado:updateProgress(ensure_networking, interactive, on_suspend)
...
function Legado:getProgress(ensure_networking, interactive)
    if not self.settings.username or not self.settings.userkey then
        if interactive then
            promptLogin()
        end
...
function Legado:_onCloseDocument()
...
function Legado:schedulePeriodicPush()
...
function Legado:_onPageUpdate(page)
    if page == nil then
        return
    end
...
function Legado:_onNetworkConnected()
...
function Legado:_onNetworkDisconnecting()
    logger.dbg("KOSync: onNetworkDisconnecting")
    -- Network is supposed to be on already, don't wrap this in willRerunWhenOnline
    self:updateProgress(false, false)
end

function Legado:onKOSyncPushProgress()
    self:updateProgress(true, true)
end

function Legado:onKOSyncPullProgress()
    self:getProgress(true, true)
end

function Legado:registerEvents()
    if self.settings.auto_sync then
